...
class gemv extends test_util.Tests {
  ...
  val scal = implN(n =>
    fun(xs => fun(a => mapSeq(fun(x => a * x), xs))) :: (ArrayType(n, f32) ->: f32 ->: ArrayType(
      n,
      f32
    ))
  )
  val dot =
    fun(xs => fun(ys => zip(xs, ys) |> mapSeq(mult) |> reduceSeq(add, l(0.0f))))
  ...
  test("High level gemv type inference works") {
    ...
    val M = typed.t
      .asInstanceOf[NatDepFunType[_ <: Type]]
      .t
      .asInstanceOf[NatDepFunType[_ <: Type]]
      .x
    assertResult(
