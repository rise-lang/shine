...
object Type {
  def fromNamed(t: rct.Type, bound: Expr.Bound): Type = {
    Type(t match {
  ...
  def toNamed(t: Type, bound: Expr.Bound): rct.Type = {
    t.node match {
...
object DataType {
  def fromNamed(dt: rct.DataType, bound: Expr.Bound): DataType = {
    DataType(dt match {
      ...
      case _: rct.DepArrayType | _: rct.DepPairType[_] | _: rct.NatToDataApply | _: rct.FragmentType =>
        throw new Exception(s"did not expect $dt")
    })
  }

  def toNamed(dt: DataType, bound: Expr.Bound): rct.DataType = {
    dt.node match {
