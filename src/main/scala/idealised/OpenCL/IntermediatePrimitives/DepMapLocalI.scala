...
final case class DepMapLocalI(dim:Int)(n: Nat,
  ...
  (Nat,
    ...
    Phrase[ExpType], Phrase[AccType]) => DepMapLocalI = DepMapLocalI(dim)
  ...
  override def substituteImpl(env: SubstituteImplementations.Environment)(implicit context: TranslationContext): Phrase[CommandType] = {
    parForNatLocal(dim)(n, i2, dt2, out, idx => a => SubstituteImplementations(f(idx)(in `@d` idx)(a), env))
  }
}