...
import rise.core.util.traversal.{Continue, Result}
...
package object dsl {
  ...
  implicit def primitiveBuilderToPrimitive(pb: Builder): dsl.ToBeTyped[Primitive] =
      pb.apply
  ...
  def app(f: ToBeTyped[Expr], e: ToBeTyped[Expr]): ToBeTyped[exprs.App] =
  ...
  def literal(d: semantics.Data): ToBeTyped[Literal] = toBeTyped(exprs.Literal(d))
