...
object oclRunPrimitive extends Builder {
  private final case class Primitive()(override val t: ExprType = TypePlaceholder) extends rise.core.Primitive {
    ...
    override def typeScheme: ExprType = expl { (ls1: Nat) => expl { (ls2: Nat) => expl { (ls3: Nat) => expl { (gs1: Nat) => expl { (gs2: Nat) => expl { (gs3: Nat) => impl { (t: DataType) => t ->: t } } } } } } }
  }
  override def toString: String = "oclRunPrimitive"
