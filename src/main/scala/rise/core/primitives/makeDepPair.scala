...
object makeDepPair extends Builder {
  private final case class Primitive()(override val t: ExprType = TypePlaceholder) extends rise.core.Primitive {
    ...
    override def typeScheme: ExprType = impl { (fdt: NatToData) => expl { (n: Nat) => NatToDataApply(fdt, n) ->: (Nat ** ((m: Nat) => NatToDataApply(fdt, m))) } }
  }
  override def toString: String = "makeDepPair"
