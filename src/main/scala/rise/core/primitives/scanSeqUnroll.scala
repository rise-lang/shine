...
object scanSeqUnroll extends Builder {
  private final case class Primitive()(override val t: Type = TypePlaceholder) extends rise.core.Primitive {
    ...
    override def typeScheme: Type = impl { (n: Nat) => impl { (s: DataType) => impl { (t: DataType) => (s ->: t ->: t) ->: t ->: ArrayType(n, s) ->: ArrayType(n, t) } } }
  }
  override def toString: String = "scanSeqUnroll"
