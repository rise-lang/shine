...
class algorithmic extends test_util.Tests {
  ...
  def loopInterchange = rise.elevate.strategies.tiling.loopInterchange(using default.RiseTraversable)
  def loopInterchangeAtLevel = rise.elevate.strategies.tiling.loopInterchangeAtLevel(using default.RiseTraversable)
        ...
        fun(ArrayType(K, ArrayType(N, f32)))(b =>
          a |> map(fun(ak =>
            transpose(b) |> map(fun(bk =>
              zip(ak)(bk) |>
                reduceSeq(fun((acc, y) => acc + (fst(y) * snd(y))))(
                  lf32(0.0f)))))))))))
