...
object vectorize {
  ...
  // _ -> asVector >> asScalar
  def after(n: Nat): Strategy[Rise] = rule("after", e => e.t match {
    // FIXME: m + n hack
  ...
  def roundUpAfter(n: Nat): Strategy[Rise] = rule("roundUpAfter", e => e.t match {
  ...
  def alignedAfter(n: Nat): Strategy[Rise] = rule("alignedAfter", e => e.t match {
    // FIXME: m + n hack
    case ArrayType(m, _: ScalarType) if (m + n) % n == (0: Nat) =>
  ...
  // asScalar >> asVector -> _
  def asScalarAsVectorId: Strategy[Rise] = rule("asScalarAsVectorId", {
    case e @ App(v, App(asScalar(), in)) if isAsVector(v) && e.t =~= in.t =>
      Success(in)
  })

  // map (reduce f init) >> asVector -> asVector >> map (reduce f init)
  def beforeMapReduce: Strategy[Rise] = rule("beforeMapReduce", {
  ...
  def beforeMapDot: Strategy[Rise] = rule("beforeMapDot", {
  ...
  def beforeMap: Strategy[Rise] = rule("beforeMap", {
  ...
  def asScalarOutsidePair: Strategy[Rise] = rule("asScalarOutsidePair", {
  ...
  // -> pair a b >> mapFst asScalar >> mapSnd asScalar
  def asScalarOutsideZip: Strategy[Rise] = rule("asScalarOutsideZip", {
    case e @ App(App(makePair(), App(asScalar(), a)), App(asScalar(), b)) =>
      Success((makePair(a)(b) |> mapFst(asScalar) |> mapSnd(asScalar)) !: e.t)
  })

  // padEmpty (p*v) (asScalar in) -> asScalar (padEmpty p in)
  def padEmptyBeforeAsScalar: Strategy[Rise] = rule("padEmptyBeforeAsScalar", {
  ...
  def padEmptyBeforeAsVector: Strategy[Rise] = rule("padEmptyBeforeAsVector", {
  ...
  def alignSlide: Strategy[Rise] = rule("alignSlide", {
  ...
  def mapAfterShuffle: Strategy[Rise] = rule("mapAfterShuffle", {
  ...
  def padEmptyBeforeZipAsVector: Strategy[Rise] = rule("padEmptyBeforeZipAsVector", {
