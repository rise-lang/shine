...
object algorithmic {
  ...
  def splitJoin: Nat => Strategy =
    n => {
      ...
      case _ => throw NotFound(splitJoin(n))
    }
  ...
  def mapFusion: Strategy = {
    case Apply(Apply(`map`, f), Apply(Apply(`map`, g), arg)) =>
      map(g >> f)(arg)
    case _ => throw NotFound(mapFusion)
  }
  ...
  def `*S >> T -> T >> S >> *T`: Strategy = {
    ...
    case _ => throw NotFound(`*S >> T -> T >> S >> *T`)
  }
