...
import shine.DPIA._
import shine.OpenCL.IntermediatePrimitives.OpenCLReduceByIndexGlobalI

import scala.xml.Elem

final case class OpenCLReduceByIndexGlobal(n: Nat,
                                           k: Nat,
  ...
  override def visitAndRebuild(fun: VisitAndRebuild.Visitor): Phrase[ExpType] = {
    OpenCLReduceByIndexGlobal(fun.nat(n), fun.nat(k), fun.addressSpace(histAddrSpace), fun.data(dt),
  ...
  override def continuationTranslation(C: Phrase[ExpType ->: CommType])(
    ...
    con(input)(λ(expT(n`.`PairType(IndexType(k), dt), read))(X =>
      OpenCLReduceByIndexGlobalI(n, k, histAddrSpace, dt,
        λ(expT(dt, read))(x =>
          λ(expT(dt, read))(y =>
