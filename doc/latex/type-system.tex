...
\begin{figure}[H]
  ...
  \begin{minipage}{1.0\linewidth} \begin{tabular*}{\linewidth}{>{$}l<{$}@{\hspace{0.4em}}>{$}c<{$}>{$}l<{$}}
        ...
        \prim{drop}&:&(n~m : \natkind) \to (\dt : \datatype) \to \tyexp[(m + n).\dt] \to \tyexp[m.\dt] \\ % TODO? differs from implementation
        \prim{take}&:&(n~m : \natkind) \to (\dt : \datatype) \to \tyexp[m.\dt] \to \tyexp[n.\dt] \\ % TODO? ensure n <= m by using addition instead?
