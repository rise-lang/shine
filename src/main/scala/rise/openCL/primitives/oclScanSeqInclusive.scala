...
object oclScanSeqInclusive extends Builder {
  private final case class Primitive()(override val t: ExprType = TypePlaceholder) extends rise.core.Primitive {
    ...
    override def typeScheme: ExprType = expl { (a: AddressSpace) => impl { (n: Nat) => impl { (s: DataType) => impl { (t: DataType) => (s ->: t ->: t) ->: t ->: ArrayType(n, s) ->: ArrayType(n + 1, t) } } } }
  }
  override def toString: String = "oclScanSeqInclusive"
