...
/**
  ...
  * @param fragmenetKind kind of the fragment ({@link FragmentType#fragmentKind})
...
final case class WmmaLoad(rows: Nat,
                          ...
                          d3: Nat,
                          dataType: DataType,
                          fragmenetKind: FragmentKind,
                          layout: MatrixLayout,
                          matrixTile: Phrase[ExpType],
                          fragment: Phrase[AccType]
                         ) extends CommandPrimitive {
  fragment :: ExpType(FragmentType(rows, columns, d3, dataType, fragmenetKind, layout), write)
