...
final case class OpenCLIterate(
  a: AddressSpace,
  n: Nat,
  m: Nat,
  k: Nat,
  ...
  override def visitAndRebuild(
    ...
    OpenCLIterate(fun.addressSpace(a), fun.nat(n), fun.nat(m), fun.nat(k),
      fun.data(dt), VisitAndRebuild(f, fun), VisitAndRebuild(array, fun))
  }
  ...
  override def xmlPrinter: Elem = {
    val l = f.t.x
    <oclIterate a={ToString(a)} n={ToString(n)} m={ToString(m)}
                k={ToString(k)} dt={ToString(dt)}>
      <f type={ToString(
