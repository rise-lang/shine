...
object fromRise {
      ...
      x match {
        ...
        case dt: lt.DataType =>
          DepApply[DataKind, PhraseType](
            expression(f)
              .asInstanceOf[Phrase[DepFunType[DataKind, PhraseType]]],
            dataType(dt)
          )
        case a: lt.AddressSpace =>
          DepApply[AddressSpaceKind, PhraseType](
            expression(f)
              .asInstanceOf[Phrase[DepFunType[AddressSpaceKind, PhraseType]]],
            addressSpace(a)
          )
      }
  ...
  def primitive(p: l.Primitive, t: lt.Type): Phrase[_ <: PhraseType] = {
    ...
    (p, t) match {
      case (core.PrintType(msg), lt.FunType(lt: lt.DataType, _)) =>
        val t = dataType(lt)
      ...
      case (
        ...
        DepLambda[NatKind](natIdentifier(n))(
          fun[ExpType](ExpType(NatType, read), e => AsIndex(n, e))
        )

      case (
          core.Map(),
          lt.FunType(
            lt.FunType(_, lb: lt.DataType),
      ...
      case (
          ...
          lt.FunType(
            _,
            lt.FunType(lb: lt.DataType, lt.FunType(lt.ArrayType(n, la), _))
          )
          ) =>
        val a = dataType(la)
        val b = dataType(lb)
        fun[ExpType ->: ExpType ->: ExpType](
          exp"[$b, $read]" ->: exp"[$a, $read]" ->: exp"[$b, $write]",
          f =>
            fun[ExpType](
              exp"[$b, $read]",
              i =>
      ...
      case (
        ...
        val b = dataType(lb)
        fun[ExpType ->: ExpType ->: ExpType](
          exp"[$a, $read]" ->: exp"[$b, $read]" ->: exp"[$b, $write]",
          f =>
            fun[ExpType](
              exp"[$b, $write]",
              i =>
      ...
      case (
        ...
        fun[ExpType](
          exp"[$n.${NatToDataLambda(n, (i: NatIdentifier) => ArrayType(lenF(i), a))}, $read]",
          e => DepJoin(n, lenF, a, e)
        )

      case (core.Join(), lt.FunType(lt.ArrayType(n, lt.ArrayType(m, la)), _)) =>
        val a = dataType(la)
        val w = read // TODO
        fun[ExpType](exp"[$n.$m.$a, $w]", e => Join(n, m, w, a, e))
      ...
      case (
        ...
        val w = read // TODO
        DepLambda[NatKind](natIdentifier(n))(
          fun[ExpType](exp"[$mn.$a, $w]", e => Split(n, m, w, a, e))
        )
      ...
      case (
        ...
        DepLambda[NatKind](natIdentifier(sz))(
          DepLambda[NatKind](natIdentifier(sp))(
            fun[ExpType](exp"[$insz.$a, $read]", e => Slide(n, sz, sp, a, e))
          )
        )
      ...
      case (
        ...
        DepLambda[NatKind](natIdentifier(sz))(
          DepLambda[NatKind](natIdentifier(sp))(
            fun[ExpType ->: ExpType](
              ExpType(s, read) ->: ExpType(s, write),
              write_dt1 =>
                fun[ExpType ->: ExpType](
                  exp"[$sz.$s, $read]" ->: ExpType(t, write),
                  f =>
                    fun[ExpType](
                      exp"[$insz.$s, $read]",
      ...
      case (
          core.Reorder(),
          lt.FunType(_, lt.FunType(_, lt.FunType(lt.ArrayType(n, la), _)))
          ) =>
        val a = dataType(la)
        fun[ExpType ->: ExpType](
          exp"[idx($n), $read]" ->: exp"[idx($n), $read]",
          idxF =>
            fun[ExpType ->: ExpType](
              exp"[idx($n), $read]" ->: exp"[idx($n), $read]",
              idxFinv =>
      ...
      case (
          core.Gather(),
          lt.FunType(lt.ArrayType(m, _), lt.FunType(lt.ArrayType(n, la), _))
          ) =>
        val a = dataType(la)
        fun[ExpType](
          exp"[$m.idx($n), $read]",
          y => fun[ExpType](exp"[$n.$a, $read]", x => Gather(n, m, a, y, x))
        )
      ...
      case (
         ...
         */
        fun[ExpType](exp"[$n.$m.$a, $read]", e => Transpose(n, m, a, e))
      ...
      case (
        ...
        val w = read // TODO
        DepLambda[NatKind](natIdentifier(n))(
          fun[ExpType](exp"[$nm.$a, $w]", e => Take(n, m, w, a, e))
        )
      ...
      case (
        ...
        val w = read // TODO
        DepLambda[NatKind](natIdentifier(n))(
          fun[ExpType](exp"[$nm.$a, $w]", e => Drop(n, m, w, a, e))
        )
      ...
      case (
          ...
          lt.DepFunType(
            ...
            lt.DepFunType(
              r: lt.NatIdentifier,
              lt.FunType(_, lt.FunType(lt.ArrayType(n, la), _))
            )
          )
          ) =>
        val a = dataType(la)
        DepLambda[NatKind](natIdentifier(l))(
          DepLambda[NatKind](natIdentifier(r))(
            fun[ExpType](
              exp"[$a, $read]",
              cst =>
                fun[ExpType](exp"[$n.$a, $read]", e => Pad(n, l, r, a, cst, e))
            )
          )
        )
      ...
      case (
        ...
        DepLambda[NatKind](natIdentifier(l))(
          DepLambda[NatKind](natIdentifier(r))(
            fun[ExpType](exp"[$n.$a, $read]", e => PadClamp(n, l, r, a, e))
          )
        )
      ...
      case (
        ...
        val a = dataType(la)
        val b = dataType(lb)
        fun[ExpType](exp"[$n.($a x $b), $read]", e => Unzip(n, a, b, e))

      case (
          core.Zip(),
          lt.FunType(lt.ArrayType(n, la), lt.FunType(lt.ArrayType(_, lb), _))
          ) =>
        val a = dataType(la)
        val b = dataType(lb)
        fun[ExpType](
          exp"[$n.$a, $read]",
          x => fun[ExpType](exp"[$n.$b, $read]", y => Zip(n, a, b, x, y))
        )

      case (core.Fst(), lt.FunType(lt.PairType(la, lb), _)) =>
        val a = dataType(la)
        val b = dataType(lb)
      ...
      case (
          core.MapFst(),
          lt.FunType(
            lt.FunType(la: lt.DataType, la2: lt.DataType),
        ...
        val b = dataType(lb)
        fun[ExpType ->: ExpType](
          exp"[$a, $read]" ->: exp"[$a2, $read]",
          f =>
            fun[ExpType](exp"[($a x $b), $read]", e => MapFst(a, b, a2, f, e))
        )

      case (core.Snd(), lt.FunType(lt.PairType(la, lb), _)) =>
        val a = dataType(la)
        val b = dataType(lb)
      ...
      case (
          core.MapSnd(),
          lt.FunType(
            lt.FunType(lb: lt.DataType, lb2: lt.DataType),
      ...
      case (
          core.Pair(),
          lt.FunType(la: lt.DataType, lt.FunType(lb: lt.DataType, _))
          ) =>
        val a = dataType(la)
        val b = dataType(lb)
        fun[ExpType](
          exp"[$a, $read]",
          x => fun[ExpType](exp"[$b, $read]", y => Pair(a, b, x, y))
        )

      case (core.Idx(), lt.FunType(_, lt.FunType(lt.ArrayType(n, la), _))) =>
        val a = dataType(la)
        fun[ExpType](
          exp"[idx($n), $read]",
          i =>
      ...
      case (core.Cast(), lt.FunType(la: lt.BasicType, lb: lt.BasicType)) =>
        val a = basicType(la)
        val b = basicType(lb)
        fun[ExpType](ExpType(a, read), x => Cast(a, b, x))
      ...
      case (
        ...
        val b = dataType(lb)
        fun[ExpType ->: ExpType](
          exp"[$a, $read]" ->: exp"[$b, $read]",
          f => fun[ExpType](ExpType(a, read), x => Let(a, b, x, f))
        )
      ...
      case (
        ...
        DepLambda[AddressSpaceKind](a)(
          DepLambda[NatKind](natIdentifier(k))(
            fun[`(nat)->:`[ExpType ->: ExpType]](
              l `()->:` (exp"[$ln.$dt, $read]" ->: exp"[$l.$dt, $read]"),
              f =>
                fun[ExpType](
                  exp"[$insz.$dt, $read]",
      ...
      case (
        ...
        val a = scalarType(la)
        DepLambda[NatKind](natIdentifier(n))(
          fun[ExpType](exp"[$mn.$a, $read]", e => AsVector(n, m, a, e))
        )

      case (
          core.AsVectorAligned(),
          lt.DepFunType(
            n: lt.NatIdentifier,
            lt.FunType(lt.ArrayType(mn, la: lt.ScalarType), lt.ArrayType(m, _))
          )
        ...
        val a = scalarType(la)
        DepLambda[NatKind](natIdentifier(n))(
          fun[ExpType](exp"[$mn.$a, $read]", e => AsVectorAligned(n, m, a, e))
        )
      ...
      case (
        ...
        val a = scalarType(la)
        fun[ExpType](
          ExpType(ArrayType(m, VectorType(n, a)), read),
          e => AsScalar(m, n, a, e)
        )

      case (
          core.VectorFromScalar(),
          lt.FunType(_, lt.VectorType(n, la: lt.ScalarType))
          ) =>
        val a = scalarType(la)
        fun[ExpType](ExpType(a, read), e => VectorFromScalar(n, a, e))

      case (core.IndexAsNat(), lt.FunType(lt.IndexType(n), lt.NatType)) =>
        fun[ExpType](exp"[idx($n), $read]", e => IndexAsNat(n, e))

      case (
          ocl.ToMem(),
          lt.DepFunType(
            las: lt.AddressSpaceIdentifier,
        ...
        DepLambda[AddressSpaceKind](as)(
          fun[ExpType](exp"[$a, $write]", e => To(as, a, e))
        )
  ...
  private def makeMap(
    ...
    val b = dataType(lb)
    fun[ExpType ->: ExpType](
      ExpType(a, read) ->: ExpType(b, write),
      f => fun[ExpType](exp"[$n.$a, $read]", e => map(n, a, b, f, e))
    )
  }
  ...
  def wrapForeignFun(
    ...
    if (i < intTs.length) {
      fun[ExpType](
        ExpType(intTs(i), read),
        a => wrapForeignFun(decl, intTs, outT, args :+ a)
      )
    } else {
  ...
  def wrapArray(
    ...
    t match {
      ...
      case lt.FunType(in: lt.DataType, t2) =>
        fun[ExpType](
          ExpType(dataType(in), read),
          e => wrapArray(t2, elements :+ e)
        )
