...
object separableConvolution2D {
  ...
  // 2 4 2 ~ 2 x 1 2 1
  // 1 2 1   1
  val binomialWeights2d: Expr =
    weights2d(1.0f / 16.0f, Seq(Seq(1, 2, 1), Seq(2, 4, 2), Seq(1, 2, 1)))
  val binomialWeightsV: Expr = weights1d(1.0f / 4.0f, Seq(1, 2, 1))
  ...
  // -2  0 +2  ~  2 x -1  0 +1
  // -1  0 +1     1
  val sobelXWeights2d: Expr =
    weights2d(1.0f / 8.0f, Seq(Seq(-1, 0, +1), Seq(-2, 0, +2), Seq(-1, 0, +1)))
  val sobelXWeightsV: Expr = weights1d(1.0f / 4.0f, Seq(1, 2, 1))
  val sobelXWeightsH: Expr = weights1d(1.0f / 2.0f, Seq(-1, 0, +1))
  // Sy:
  // -1 -2 -1     -1
  //  0  0  0  ~   0 x 1 2 1
  // +1 +2 +1     +1
  val sobelYWeights2d: Expr =
    weights2d(1.0f / 8.0f, Seq(Seq(-1, -2, -1), Seq(0, 0, 0), Seq(+1, +2, +1)))
  ...
  val dot: Expr =
    fun(a => fun(b => zip(a)(b) |> map(mulT) |> reduce(add)(l(0.0f))))
  val dotSeq: Expr =
    fun(a => fun(b => zip(a)(b) |> map(mulT) |> reduceSeq(add)(l(0.0f))))
  val dotSeqUnroll: Expr =
    fun(a => fun(b => zip(a)(b) |> map(mulT) |> reduceSeqUnroll(add)(l(0.0f))))
  val weightsSeqVecUnroll: Expr = fun(weights =>
    fun(vectors =>
      zip(map(vectorFromScalar)(weights))(vectors) |>
        map(mulT) |> oclReduceSeqUnroll(AddressSpace.Private)(add)(
  ...
  val factorised: Expr = fun(3 `.` f32)(weightsV =>
    fun(3 `.` f32)(weightsH =>
      padClamp2D(1) >> slide2D(3, 1) >>
        map(map(map(dot(weightsH)) >> dot(weightsV)))
    )
  )
  val factorisedSeq: Expr = fun(3 `.` f32)(weightsV =>
    fun(3 `.` f32)(weightsH =>
      padClamp2D(1) >> slide2D(3, 1) >>
        mapSeq(
          mapSeq(map(dotSeqUnroll(weightsH)) >> dotSeqUnroll(weightsV))
        )
    )
  )

  val separated: Expr = fun(3 `.` f32)(weightsV =>
    fun(3 `.` f32)(weightsH => {
  ...
  val separatedSeq: Expr = fun(3 `.` f32)(weightsV =>
    fun(3 `.` f32)(weightsH => {
  ...
  val scanline: Expr = fun(3 `.` f32)(weightsV =>
    fun(3 `.` f32)(weightsH =>
  ...
  val scanlineSeq: Expr = fun(3 `.` f32)(weightsV =>
    fun(3 `.` f32)(weightsH =>
  ...
  val scanlinePar: Expr = fun(3 `.` f32)(weightsV =>
    fun(3 `.` f32)(weightsH =>
      map(
        implN(w =>
          fun(w `.` f32)(x =>
  ...
  val regRotSeq: Expr = fun(3 `.` f32)(weightsV =>
    fun(3 `.` f32)(weightsH =>
      padClamp2D(1) >> slide(3)(1) >> mapSeq(
        transpose >>
  ...
  val regRotPar: Expr = fun(3 `.` f32)(weightsV =>
    fun(3 `.` f32)(weightsH => {
  ...
  def computeGold(
    ...
    for (y <- input.indices) {
      ...
      for (x <- r1.indices) {
        ...
        output(y)(x) =
          weights(0)(0) * r0(c0) + weights(0)(1) * r0(c1) + weights(0)(2) * r0(
            c2
          ) +
            weights(1)(0) * r1(c0) + weights(1)(1) * r1(c1) + weights(1)(2) * r1(
            c2
          ) +
            weights(2)(0) * r2(c0) + weights(2)(1) * r2(c1) + weights(2)(2) * r2(
            c2
          )
      }
    }
    ...
    weights match {
      case Literal(ArrayData(a)) =>
        computeGold(
          ...
          input,
          a.map(r =>
              r.asInstanceOf[ArrayData]
                .a
                .map(x => x.asInstanceOf[FloatData].f)
                .toArray
            )
            .toArray
        )
