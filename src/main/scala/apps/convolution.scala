...
object convolution {
  ...
  private val dotElemWeights = fun(
    (weights, elem) =>
  ...
  val blurXTiled2D: Expr = nFun(
    n =>
      fun((n `.` n `.` f32) ->: (17 `.` f32) ->: (n `.` n `.` f32))(
        (matrix, weights) =>
  ...
  def unpadEmpty(l: Nat, r: Nat): Expr =
    implN(
      n => implDT(t => drop(l) >> (take(n) :: ((n + r) `.` t) ->: (n `.` t)))
    )

  val blurYTiled2DTiledLoadingTransposed: Expr = nFun(
    n =>
      fun((n `.` n `.` f32) ->: (17 `.` f32) ->: (n `.` n `.` f32))(
        (matrix, weights) =>
          unslide2D o mapWorkGroup(1)(
  ...
  def blurYTiled2DTiledLoadingTransposed_sizes(
    n: Int
