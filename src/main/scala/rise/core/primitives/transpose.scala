...
object transpose extends Builder {
  private final case class Primitive()(override val t: ExprType = TypePlaceholder) extends rise.core.Primitive {
    ...
    override def typeScheme: ExprType = impl { (n: Nat) => impl { (m: Nat) => impl { (t: DataType) => ArrayType(n, ArrayType(m, t)) ->: ArrayType(m, ArrayType(n, t)) } } }
  }
