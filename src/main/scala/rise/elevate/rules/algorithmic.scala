...
object algorithmic {
  ...
  def reduceMapFission()(implicit ev: Traversable[Rise]): Strategy[Rise] =
    rule("reduceMapFission", {
  ...
  def liftId()(using ev: Traversable[Rise]): Strategy[Rise] = rule("liftId", {
    case App(id() ::: FunType(ArrayType(_, _), _), arg) => Success(DFNF()(using ev)((map(id) $ arg)).get)
  })
  ...
  // x -> join (slide 1 1 x)
  def slideAfter: Strategy[Rise] =
    rule("slideAfter", e => Success(join(slide(1: Nat)(1: Nat)(e)) !: e.t))

  def slideAfter2: Strategy[Rise] =rule("slideAfter2",
    e => Success(map(fun(x => x `@` lidx(0, 1)))(slide(1: Nat)(1: Nat)(e)) !: e.t))
  ...
  def freshLambdaIdentifier()(implicit ev: Traversable[Rise]): Strategy[Rise] =
    rule("freshLambdaIdentifier", e => {
  ...
  def blockedReduce(n: Nat)(using ev: Traversable[Rise]): Strategy[Rise] =
    rule("blockedReduce", {
