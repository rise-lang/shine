...
object harrisCornerDetection {
  ...
  private val sobelX = nFun(
    ...
    h =>
      nFun(
        ...
        w =>
          fun((h `.` w `.` f32) ->: (h `.` w `.` f32))(
            a => C2D.regRotPar(C2D.sobelXWeightsV)(C2D.sobelXWeightsH)(a)
        )
    )
  )
  private val sobelY = nFun(
    hRange,
    h =>
      nFun(
        wRange,
        w =>
          fun((h `.` w `.` f32) ->: (h `.` w `.` f32))(
            a => C2D.regRotPar(C2D.sobelYWeightsV)(C2D.sobelYWeightsH)(a)
        )
    )
  )
  private val mul = nFun(
    hRange,
    h =>
      nFun(
        wRange,
        w =>
          fun((h `.` w `.` f32) ->: (h `.` w `.` f32) ->: (h `.` w `.` f32))(
            (a, b) =>
              zip(a, b) |> mapGlobal(
                fun(
                  ab =>
  ...
  private val gaussian = nFun(
    ...
    h =>
      nFun(
        ...
        w =>
          fun((h `.` w `.` f32) ->: (h `.` w `.` f32))(
            a => C2D.regRotPar(C2D.binomialWeightsV)(C2D.binomialWeightsH)(a)
        )
    )
  )
  private val coarsityVector = fun(
    sxx =>
      fun(
        sxy =>
          fun(
      ...
      nFun(
        ...
        w =>
          fun(
            ...
            (sxx, sxy, syy, kappa) =>
              zip(sxx, zip(sxy, syy)) |> mapGlobal(
                fun(
                  s =>
                    zip(asVectorAligned(4)(s._1))(
                      zip(asVectorAligned(4)(s._2._1))(
                        asVectorAligned(4)(s._2._2)
                      )
                    ) |>
                      mapSeq(fun(s => {
                        val sxx = fst(s)
  ...
  private val sobelXYMuls = nFun(
    ...
    h =>
      nFun(
        ...
        w =>
          fun(
            (h `.` w `.` f32) ->: (3 `.` h `.` w `.` f32) // (h`.`w`.`f32) x ((h`.`w`.`f32) x (h`.`w`.`f32)))
          )(input => {
            input |>
              map(
                fun(w `.` f32)(
  ...
  private val gaussianCoarsity = nFun(
    hRange,
    h =>
      nFun(
        wRange,
        w =>
          fun((3 `.` h `.` w `.` f32) ->: f32 ->: (h `.` w `.` f32))(
            (input, kappa) => {
              input |>
                map(
                  map(
  ...
  private val sobelXY = nFun(
    hRange,
    h =>
      nFun(
        wRange,
        w =>
          fun((h `.` w `.` f32) ->: (2 `.` h `.` w `.` f32))(
            input =>
              input |>
                map(
                  fun(w `.` f32)(
  ...
  private val mulGaussianCoarsity = nFun(
    hRange,
    h =>
      nFun(
        wRange,
        w =>
          fun((2 `.` h `.` w `.` f32) ->: f32 ->: (h `.` w `.` f32))(
            (input, kappa) =>
              input |>
                map(
                  map(
  ...
  private val sobelXYMulGaussianCoarsity = nFun(
    hRange,
    h =>
      nFun(
        wRange,
        w =>
          fun((h `.` w `.` f32) ->: f32 ->: (h `.` w `.` f32))(
            (input, kappa) => input // TODO
        )
    )
  )
  ...
  case class NoPipe(sobelX: KernelNoSizes,
    ...
    def run(input: Array[Array[Float]],
      ...
      val fC = coarsity.as[
        ScalaFunction `(`
          Int `,` Int `,` Array[Array[Float]] `,` Array[Array[Float]] `,` Array[
          Array[Float]
        ] `,` Float
          `)=>` Array[Float]
      ]
  ...
  case class HalfPipe1(sobelXY: KernelNoSizes,
    ...
    def run(input: Array[Array[Float]],
      ...
      val fMGC = mulGaussianCoarsity.as[
        ScalaFunction `(`
          Int `,` Int `,` Array[Array[Array[Float]]] `,` Float
          `)=>` Array[Float]
      ]
  ...
  case class FullPipe(sobelXYMulGaussianCoarsity: KernelNoSizes) {
    def run(input: Array[Array[Float]],
      ...
      val f = sobelXYMulGaussianCoarsity.as[
        ScalaFunction `(`
  ...
  private def zip2D[A, B](as: Array[Array[A]],
                          bs: Array[Array[B]]): Array[Array[(A, B)]] =
  ...
  def computeGold(h: Int,
    ...
    val ixx = computePointwiseGold(ix, { x: Float =>
      x * x
    })
    val ixy = computePointwiseGold[(Float, Float), Float](zip2D(ix, iy), {
      case (a, b) => a * b
    })
    val iyy = computePointwiseGold(iy, { x: Float =>
      x * x
    })
