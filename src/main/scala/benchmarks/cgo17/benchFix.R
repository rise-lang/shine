...
# "MM NVIDIA", "MM AMD"
benchSeq <- c("N-Body NVIDIA", "N-Body AMD", "MM NVIDIA", "MM AMD", "Mean")
data <- data %>%
...
g <- ggplot(results, aes(x=size, y=relativeTime, fill=generator)) +
  ...
  xlab("input size") +
  scale_y_continuous(name = "relative runtime performance", trans=t_shift, breaks = c(0.5, 1, 2.0), limits = c(0.25, 3)) +
