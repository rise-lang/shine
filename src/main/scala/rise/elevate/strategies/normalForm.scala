...
object normalForm {
  ...
  def BENF()(using Traversable[Rise]): Strategy[Rise] =
    strategy("BENF", normalize(etaReduction() <+ betaReduction))

  // Data-Flow-Normal-Form
  def DFNF()(using Traversable[Rise]): Strategy[Rise] =
    strategy("DFNF", BENF() `;`
  ...
  def RNF()(using Traversable[Rise]): Strategy[Rise] =
    strategy("RNF", normalize(DFNF() `;` mapLastFission()) `;` DFNF())

  // Codegen-Normal-Form (Fuse all maps)
  def CNF()(using Traversable[Rise]): Strategy[Rise] =
