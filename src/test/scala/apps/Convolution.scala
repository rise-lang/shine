...
class Convolution extends test_util.TestsWithExecutor {
  ...
  test("convolution versions produce same results") {
    ...
    test_util.runsWithSameResult(Seq(
      ...
      ("dpiaX", runKernel(gen.opencl.kernel.fromExpr("blurXTiled2D")(blurXTiled2D(N)),
    ...
    test_util.runsWithSameResult(Seq(
      ...
      ("dpiaY", runKernel(
        gen.opencl.kernel.fromExpr("blurYTiled2DTiledLoadingTransposed")(blurYTiled2DTiledLoadingTransposed(N)),
