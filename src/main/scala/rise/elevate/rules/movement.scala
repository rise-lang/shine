...
object movement {
  ...
  def `take n (zip a b) -> zip (take n a) (take n b)`: Strategy[Rise] =
    rule("take n (zip a b) -> zip (take n a) (take n b)", {
  ...
  def `drop n (zip a b) -> zip (drop n a) (drop n b)`: Strategy[Rise] =
    rule("drop n (zip a b) -> zip (drop n a) (drop n b)", {
  ...
  def `take n (select t a b) -> select t (take n a) (take n b)`: Strategy[Rise] =
    rule("take n (select t a b) -> select t (take n a) (take n b)", {
  ...
  def `drop n (select t a b) -> select t (drop n a) (drop n b)`: Strategy[Rise] =
    rule("drop n (select t a b) -> select t (drop n a) (drop n b)", {
  ...
  def `take (n+m) >> drop m -> drop m >> take n`: Strategy[Rise] =
    rule("take (n+m) >> drop m -> drop m >> take n", {
  ...
  def `drop m >> take n -> take (n+m) >> drop m`: Strategy[Rise] =
    rule("drop m >> take n -> take (n+m) >> drop m", {
  ...
  def `slide n m >> take t -> take (m * (t - 1) + n) >> slide n m`: Strategy[Rise] =
    rule("slide n m >> take t -> take (m * (t - 1) + n) >> slide n m", {
  ...
  def `slide n m >> drop d -> drop (d * m) >> slide n m`: Strategy[Rise] =
    rule("slide n m >> drop d -> drop (d * m) >> slide n m", {
  ...
  def `slide(n)(s) >> split(k) -> slide(k+n-s)(k) >> map(slide(n)(s))`: Strategy[Rise] =
    rule("slide(n)(s) >> split(k) -> slide(k+n-s)(k) >> map(slide(n)(s))", {
