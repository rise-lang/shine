...
import rise.core.primitives.{scanSeqInclusive, _}
...
class sparseDense extends test_util.TestsWithExecutor {
  ...
  def dense_to_sparse_ocl_parallel(n:Nat, m:Nat):ToBeTyped[Expr] = fun((n + 1) `.` NatType)(offsExpression => fun(n `.` m `.` f32)(array => {
  ...
  def runDenseToSparse(logicalSizes: Iterable[Int], filterEach: Int): Iterable[(Int, Int, String)]= {
    val e2 = depFun((n: Nat) => depFun((m:Nat) => dense_to_sparse_ocl_parallel_colmaj(n, m)))
    ...
    logicalSizes.map(logicalSize => {
      ...
      val matrixColmaj = matrix.transpose
      ...
      (logicalSize, filterEach, time.toString)
    })
  }
...
//      64 -> List(1024, 2048, 3072, 4096, 5120, 6144, 7168, 8092),
//      128 -> List(1024, 2048, 3072, 4096, 5120, 6144, 7168, 8092),
//      256 -> List(1024, 2048, 3072, 4096, 5120, 6144, 7168, 8092),
//      512 -> List(1024, 2048, 3072, 4096, 5120, 6144, 7168, 8092),
//      1024 -> List(1024, 2048, 3072, 4096, 5120, 6144, 7168, 8092),
//      2084 -> List(1024, 2048, 3072, 4096, 5120, 6144, 7168, 8092),
//    )

    val control = List(
      1 -> List(1024, 2048, 3072, 4096, 5120, 6144, 7168, 8092),
      2 -> List(1024, 2048, 3072, 4096, 5120, 6144, 7168, 8092),
      4 -> List(1024, 2048, 3072, 4096, 5120, 6144, 7168, 8092),
      8 -> List(1024, 2048, 3072, 4096, 5120, 6144, 7168, 8092),
      16 -> List(1024, 2048, 3072, 4096, 5120, 6144, 7168, 8092),
    )
    val results = control.map {
      case (density, sizes) =>
        val results = runDenseToSparse(sizes,  density)
        density -> results
    }
  ...
  test("BENCH: Dense to sparse ocl BIG") {
    val control = List(
      2084 -> List(16384),
    )
    val results = control.map {
  ...
  def dense_to_sparse_ocl_parallel_all_steps(n:Nat, m:Nat):ToBeTyped[Expr] =
    fun(n `.` m `.` f32)(array => {
      array |>
        mapGlobal(0)(fun(row => map(fun(x => x =/= l(0.0f)))(row) |> oclCount(AddressSpace.Private) |> indexAsNat)) |>
        oclToMem(AddressSpace.Global) |>
        split(n) |>
           mapGlobal(0)(oclScanSeqInclusive(AddressSpace.Private)(fun(x => fun(y => x + y)))(Literal(NatData(0)))) |>
        join |>
        oclToMem(AddressSpace.Global) |>
        fun(offsExpression =>
          liftNats(offsExpression)(depFun((offs: NatCollection) =>
              dpairNats(offs)(
                array |> toDepArray |>
                  depMapGlobal(0)(depFun((rowIdx:Nat) => fun(row =>
                    oclWhichMap(row |> map(fun(x => x =/= l(0.0f))))
                    (fun(nnzIdx => pair(row `@` nnzIdx)(nnzIdx)))
                    ((offs `@` (rowIdx + 1)) - (offs `@` rowIdx))
                  )
                  ))))))
  })
