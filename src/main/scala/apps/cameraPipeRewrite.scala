...
object cameraPipeRewrite {
  ...
  def idxAfterF: Strategy[Rise] = {
    case expr @ App(f, App(App(p.idx(_), i), in)) =>
  ...
  def singleInputId(ret: Int => Unit): Strategy[Rise] = p => {
    ...
    def traverse: Strategy[Rise] = { p =>
      val s = (
        ...
        case p @ App(
          App(idx(_), Literal(IndexData(Cst(i), _), _)),
          _
        ) =>
