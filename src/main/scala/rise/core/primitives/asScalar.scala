...
object asScalar extends Builder {
  private final case class Primitive()(override val t: ExprType = TypePlaceholder) extends rise.core.Primitive {
    ...
    override def typeScheme: ExprType = impl { (n: Nat) => impl { (m: Nat) => impl { (t: DataType) => ArrayType(m, VectorType(n, t)) ->: ArrayType(m * n, t) } } }
  }
