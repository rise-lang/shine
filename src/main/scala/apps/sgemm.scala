...
object sgemm {
  ...
  val mult: ToBeTyped[Expr] = impl{ dt: DataType => fun(x => x.`1` * x._2) :: ((dt x dt) ->: dt) }
  ...
  val dot: ToBeTyped[Expr] = fun(x => foreignFun("dot", vec(4, f32) ->: vec(4, f32) ->: f32)(x.`1`, x._2))
        ...
        ((a, b, c, alpha, beta) =>
          ...
          zip(a)(c) |> mapSeq(fun(ac =>
            zip(transpose(b))(ac._2) |> mapSeq(fun(bc =>
              zip(ac.`1`)(bc.`1`) |>
                reduceSeq(fun( (acc, y) => acc + (y.`1` * y._2)))(lf32(0.0f)) |>
    ...
    depFun((n: Nat, m: Nat, k: Nat) =>
      ...
      ((a, b, c, alpha, beta) =>
        ...
        zip(a)(c) |> mapSeq(fun(ac =>
          zip(transpose(b))(ac._2) |> mapSeq(fun(bc =>
            zip(ac.`1`)(bc.`1`) |>
              oclReduceSeq(AddressSpace.Private)(fun( (acc, y) => acc + (y.`1` * y._2)))(lf32(0.0f)) |>
              fun(x => (x * alpha) + (beta * bc._2))
          ))
        ))
      )
                ...
                zip(split(p3)(transpose(ac.`1`)))(split(p3)(transpose(bc.`1`))) |>
                  ...
                  fun(p235 =>
                    zip(p235)(transpose(bc._2)) |>
                      mapSeq(fun(p237 =>
                        zip(p237.`1`)(p237._2) |>
                          mapSeq(fun(p64 => (p64.`1` * alpha) + (p64._2 * beta)))
                      ))
                  ) |> transpose
              )) |> join |> transpose
          )) |> join
  ...
  val keplerBest: ToBeTyped[Expr] = {
    ...
    def redOp: ToBeTyped[Expr] = fun((8`.`32`.`8`.`4`.`f32) ->: ( (8`.`64`.`f32) x (8`.`128`.`f32) ) ->: (8`.`32`.`8`.`4`.`f32) )((p14, p15) =>
      let(p15 |> fun(p29 =>
          zip (p29.`1`) (p29._2)
            |> toLocalFun(mapLocal(1) (fun(p31 => makePair (mapLocal(0) (id) (p31.`1`)) (mapLocal(0) (id) (p31._2)) )))
            |> unzip
        ))
      be (p16 =>
        zip (p14) (split (v5) (transpose (p16.`1`)))
          |> mapLocal(1) (fun(p17 =>
          zip (p17.`1`) (split (v4) (reorderWithStride (v3/v4) (transpose (p16._2))))
            |> mapLocal(0) (fun(p18 =>
            zip (transpose (p17._2)) (transpose (p18._2))
              |> oclReduceSeq (AddressSpace.Private) (fun( (p20, p21) =>
                let (makePair (toPrivate(mapSeq (id) (p21.`1`))) (toPrivate(mapSeq (id) (p21._2))))
                be (fun(p22 =>
                    zip (p20) (p22.`1`) |> mapSeq (fun(p23 =>
                      zip (p23.`1`) (p22._2) |> mapSeq (fun(p24 =>
                        p24.`1` + (p23._2 * p24._2) )) )) ))
          ...
          |> mapWorkGroup(0)(
            ...
            |> fun(x =>
              zip (x) (split (v5) (p3._2))
                |> mapLocal(1) (fun(y =>
                zip (y.`1`) (split (v4) (reorderWithStride (v3/v4) (transpose (y._2)))) |> mapLocal(0) (fun(z =>
                  zip (z.`1`) (transpose (z._2)) |> mapSeq (fun(a =>
                    zip (a.`1`) (a._2) |> mapSeq (fun(x =>
                      (x.`1` * alpha) + (x._2 * beta) )))))))))
            |> map (fun(p4 => p4
              |> map (transpose)
              |> join
