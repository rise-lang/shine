  ...
  case class FromKernelModule(ktu: KernelModule) {
    ...
    def as[F <: FunctionHelper](localSize: LocalSize, globalSize: GlobalSize)
      ...
      hArgs: F#T => {
        ...
        val runtime = execute(kernelArgs)
    ...
    private def execute(kernelArgs: List[KernelArg]): Double = {
      val runtime = Executor.executeC(code, this.kernel.name, kernelArgs.toArray: _*)
