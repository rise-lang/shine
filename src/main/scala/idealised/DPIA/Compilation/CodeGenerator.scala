...
trait CodeGenerator[Environment, Path, Stmt, Expr, Decl, Ident, Type] {
  ...
  def generate(phrase: Phrase[CommandType]): (Seq[Decl], Stmt)
...
//                             gen: this.type): Expr
//
//  def generateAccess(dt: DataType, identifier: Ident, paths: Path, env: Environment): Expr
}