...
final case class OpenCLFunction(name: String,
  ...
  override def inferTypes: TypeInferable = {
  ...
  override def prettyPrint: String = s"(__${name}__ ${args.map(PrettyPrinter(_))})"
          ...
          exp(arg)(λ(exp"[$inT]")(e =>
            outT match {
              ...
              case RecordType(dt11, dt12) =>
                acc(fst(OpenCLFunction(name, inTs :+ inT, outT, es :+ e)))(fstAcc(dt11, dt12, A)) `;`
                  acc(snd(OpenCLFunction(name, inTs :+ inT, outT, es :+ e)))(sndAcc(dt11, dt12, A))
