...
class Acoustic3D extends test_util.Tests {
  private val N = 128
  private val M = 64
  private val O = 32
  ...
  test("acoustic stencils produce same results") {
    ...
    // do we have parameters to inject?
    //    val expression: Expr = stencilMMS.toExpr

    val expr = rise.autotune.wrapOclRun(localSize = LocalSize((32, 4)), globalSize = GlobalSize((N, M)))(stencilMSS)
    val kernel = gen.opencl.hosted("fun").fromExpr(expr)

    //    println("module: " + kernel)

    rise.autotune.Timeouts(
      codegenerationTimeout = 1000,
    ...
    val result = rise.autotune.execution.execute(
      expression = expr,
      hostCode = explorations.acoustic.hostCode,
      timeouts = rise.autotune.Timeouts(
        codegenerationTimeout = 1000,
        compilationTimeout = 1000,
        executionTimeout = 1000
      ),
      executionIterations = 11,
      speedupFactor = 10,
      execution = rise.autotune.Median
    )

    // print result to compute/generate the gold
    // read in gold from the file


    println("result: " + result)

    //    println(shine.OpenCL.Module.translateToString(kernel))

    //    val kernel = gen.opencl.kernel.asStringFromExpr(stencilMSS)
