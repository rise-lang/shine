...
object gen {
  ...
  object cuda {
    ...
    type KernelDef = CudaKernelDefinition
    ...
    def phraseToKernelDef(name: String): Phrase => KernelDef =
      CudaKernelDefinition.fromPhrase(name)

    def kernelDefToKernel(wgConfig: Option[LocalAndGlobalSize]): KernelDef => KernelModule = wgConfig match {
      case Some(LocalAndGlobalSize(localSize, globalSize)) =>
        shine.cuda.KernelModule.fromKernelDef(Some(localSize, globalSize))
      case None =>
        shine.cuda.KernelModule.fromKernelDef(None)
    }
  }
