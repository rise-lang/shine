...
object gap8hwConv7x4 extends Builder {
  private final case class Primitive()(override val t: ExprType = TypePlaceholder) extends rise.core.Primitive {
    ...
    override def typeScheme: ExprType = impl { (h: Nat) => impl { (w: Nat) => impl { (dt: DataType) => expl { (bias: Nat) => ArrayType(h, ArrayType(w, dt)) ->: ArrayType(4, ArrayType(7, dt)) ->: ArrayType(h - 3, ArrayType(w - 6, dt)) } } } }
  }
