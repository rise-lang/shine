...
object gap8hwConvv3 extends Builder {
  private final case class Primitive()(override val t: Type = TypePlaceholder) extends rise.core.Primitive {
    ...
    override def typeScheme: Type = impl { (iw: Nat) => impl { (ih: Nat) => impl { (fw: Nat) => impl { (fh: Nat) => impl { (ow: Nat) => impl { (oh: Nat) => impl { (bias: Nat) => impl { (s: DataType) => ArrayType(ih, ArrayType(iw, s)) ->: ArrayType(fh, ArrayType(fw, s)) ->: ArrayType(oh, ArrayType(ow, s)) } } } } } } } }
  }
  override def toString: String = "gap8hwConvv3"
