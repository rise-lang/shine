...
import opencl.generator.OpenCLAST.{ArithExpression, AssignmentExpression, Barrier, BinaryExpression, Block, Break, Cast, Comment, CondExpression, ExpressionStatement, ForLoop, Function, FunctionCall, GOTO, IfThenElse, Label, Literal, Load, OclAstNode, OpenCLCode, OpenCLExpression, OpenCLExtension, ParamDecl, PointerCast, RequiredWorkGroupSize, Skip, Store, StructConstructor, TupleAlias, TypeDef, UnaryExpression, VLoad, VStore, VarDecl, VarRef, VectorLiteral, WhileLoop}
import opencl.generator._
import opencl.ir.{IntPtr, PtrType}

//Moved here from the lift compiler dependency, as some modifications were needed
...
object OpenCLPrinter {
  ...
  def toString(t: Type, seenArray: Boolean = false) : String = {
    t match {
      ...
      case p: PtrType => p.addressSpace + " " + p.name + "*"
  ...
  def toString(v: VarRef): String = {
    v.name
  }

  def toString(v: VarDecl): String = {
    v.name
  }
}
...
class OpenCLPrinter {
  ...
  private def print(node: OclAstNode): Unit = node match {
    ...
    case l: VLoad          => print(l)
    case s: VStore         => print(s)
    case l: Load          => print(l)
    ...
    case l: Literal       => sb ++= l.code
    case b: BinaryExpression => print(b)
    case u: UnaryExpression => print(u)
    case s: Skip =>
  ...
  private def print(c: PointerCast): Unit = {
    ...
    print(OpenCLPrinter.toString(c.v))
    print(")")
  ...
  private def print(l: VLoad): Unit = {
  ...
  private def print(s: VStore): Unit = {
  ...
  private def print(vd: VarDecl): Unit = vd.t match {
          ...
          if (vd.length > 0) {
            for (i <- 0 until vd.length.toInt)
              println(OpenCLPrinter.toString(Type.getValueType(vd.t)) + " " +
                OpenCLPrinter.toString(vd) + "_" +
                OpenCLPrinter.toString(i) + ";")
          } else {
            print(OpenCLPrinter.toString(Type.getValueType(vd.t)) + " " +
              OpenCLPrinter.toString(vd) + "[" +
              Type.getLengths(vd.t).map(_.evalInt).product +
              "];")
          }

        case opencl.ir.LocalMemory if vd.length != 0 =>
          val baseType = Type.getBaseType(vd.t)
          val declaration =
            s"${vd.addressSpace} ${OpenCLPrinter.toString(baseType)} " +
              s"${OpenCLPrinter.toString(vd)}[${vd.length}]"
    ...
    case _ =>
      ...
      vd.t match {
        case IntPtr =>
          if(vd.addressSpace == opencl.ir.GlobalMemory)
            print(vd.addressSpace + " ")
        case _ =>
      }
      if(vd.addressSpace == opencl.ir.LocalMemory)
        print(vd.addressSpace + " ")
      print(s"${OpenCLPrinter.toString(vd.t)} ${vd.name}")
      if(vd.init != null) {
        print(s" = ")
        print(vd.init)
      }
  ...
  private def print(b: Barrier): Unit = println (b.mem.addressSpace match {
    case opencl.ir.GlobalMemory => "barrier(CLK_GLOBAL_MEM_FENCE);"
    case opencl.ir.LocalMemory => "barrier(CLK_LOCAL_MEM_FENCE);"
  ...
  private def print(u: UnaryExpression): Unit = {
    print(s"(${u.op}")
    print(u.e)
    print(")")
  }

  private def print(b: BinaryExpression): Unit = {
    print("(")
    print(b.lhs)
    print(s" ${b.op} ")
    print(b.rhs)
