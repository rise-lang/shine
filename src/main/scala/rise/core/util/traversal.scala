      ...
      v.visitExpr(expr) match {
        ...
        case c: Continue[Expr] =>
          ...
          c.value match {
            ...
            case l: Literal =>
              l.d match {
                case NatData(n) => exprs.Literal(NatData(v.visitNat(n).value))
                case IndexData(i, n) =>
                  exprs.Literal(IndexData(v.visitNat(i).value, v.visitNat(n).value))
                case _ => l
              }
        ...
        case Continue(c, v) =>
          c match {
            ...
            case l: Literal =>
              l.d match {
                case NatData(n) =>
                  v.visitNat(n).map(r => exprs.Literal(NatData(r)))
                case IndexData(i, n) =>
                  chainN(v.visitNat(i), n).map(r =>
                    exprs.Literal(IndexData(r._1, r._2))
                  )
                case _ => Continue(l, v)
              }
