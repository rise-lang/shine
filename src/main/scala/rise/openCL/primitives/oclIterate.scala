...
object oclIterate extends Builder {
  private final case class Primitive()(override val t: ExprType = TypePlaceholder) extends rise.core.Primitive {
    ...
    override def typeScheme: ExprType = expl { (a: AddressSpace) => impl { (n: Nat) => impl { (m: Nat) => expl { (k: Nat) => impl { (t: DataType) => expl { (l: Nat) => ArrayType(l * n, t) ->: ArrayType(l, t) } ->: ArrayType(m * n.pow(k), t) ->: ArrayType(m, t) } } } } }
  }
  override def toString: String = "oclIterate"
