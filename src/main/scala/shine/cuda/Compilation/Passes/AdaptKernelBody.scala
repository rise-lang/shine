...
object AdaptKernelBody {
  ...
  def adapt(s: C.AST.Stmt): (C.AST.Block, Long) =
    adaptBlock(C.AST.Block(immutable.Seq(s)))

  def adaptBlock: C.AST.Block => (C.AST.Block, Long) =
  ...
  private object moveLocalMemoryVariableDeclarations {
    def apply(body: C.AST.Block): (C.AST.Block, Long) = {
      //Calculate SharedMemory usage
      //TODO at compile time not known dynamicSharedMemory
