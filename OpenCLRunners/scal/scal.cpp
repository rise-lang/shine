...
int main(int argc, char **argv) {
  ...
  try {
    if (argc != 4) {
      std::cout << "Usage: 'scal kernelFile platformId deviceId'\n";
      std::abort();
    }
    ...
    for (int i = 0; i < 10; ++i) {
      ...
      queue.enqueueWriteBuffer(input, CL_TRUE, 0, size * sizeof(float),
                               x.data());
    ...
    // check result
    if (x.size() != z.size()) {
      std::cout << "input and output size do not match!\n";
      std::abort();
    }
    auto epsilon = 0.001f;
    auto errors = 0;
    for (int i = 0; i < z.size(); ++i) {
      if ((alpha * x[i]) - z[i] > epsilon) {
        ++errors;
      }
    }

    if (errors) {
      std::cout << errors << " many elements differ from gold.\n";
      std::abort();
    }
