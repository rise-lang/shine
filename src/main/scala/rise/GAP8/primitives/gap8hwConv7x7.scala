...
object gap8hwConv7x7 extends Builder {
  private final case class Primitive()(override val t: ExprType = TypePlaceholder) extends rise.core.Primitive {
    ...
    override def typeScheme: ExprType = expl { (bias: Nat) => impl { (ih: Nat) => impl { (iw: Nat) => impl { (oh: Nat) => impl { (ow: Nat) => impl { (s: DataType) => ArrayType(ih, ArrayType(iw, s)) ->: ArrayType(7, ArrayType(7, s)) ->: ArrayType(oh, ArrayType(ow, s)) } } } } } }
  }
