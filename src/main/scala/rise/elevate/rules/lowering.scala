...
object lowering {
  ...
  def isComputation()(implicit ev: Traversable[Rise]): Strategy[Rise] =
    rule("isComputation", e => {
  ...
  def toMemAfter: Strategy[Rise] =
    rule("toMemAfter", e => Success((preserveType(e) |> p.toMem) !: e.t))
