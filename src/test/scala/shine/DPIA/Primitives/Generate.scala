...
class Generate extends test_util.Tests {
  ...
  test(
    "Very simplistic generate, using index and maximum index size" +
  ...
  ignore(
    "Syntactically correct code for complex Generate can be generated in C."
  ) {
    val N = 8
    val LPrevIter: Nat = 1
    val p = 2
    ...
    val id = fun(x => x)
    val generateSth =
      fun(ArrayType(N, f32))(_ => reorderedB >> mapSeq(mapSeq(mapSeq(id))))
