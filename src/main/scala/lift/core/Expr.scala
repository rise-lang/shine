...
import lift.core.DSL.implT

sealed abstract class Expr(val t: Type) {
  def setType(t: Type): Expr
}

final case class Identifier(name: String)
                           (override val t: Type = implT(identity)) extends Expr(t) {
...
final case class DepLambda[K <: Kind](x: K#I, e: Expr)
                                     (override val t: Type = implT(identity))
                                     (implicit val kn: KindName[K]) extends Expr(t) {
...
final case class Literal(d: semantics.Data)
                        (override val t: Type = implT(identity)) extends Expr(t) {
  override def toString: String = s"$d"

  override def setType(t: Type): Literal = this.copy(d)(t)
}
