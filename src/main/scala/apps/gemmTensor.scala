...
import rise.openCL.DSL.{oclReduceSeq, toPrivate, toLocal}
...
//            tensorMMA(
//              abTiles._1 |> transpose |> toFragmentA(Row_Major, k),
//              abTiles._2 |> toFragmentB(Col_Major, k),
//              cTile)))
//
//          (bColumnsTCT._2 |>
//            transpose |>
//            toFragmentAccumulator(Row_Major, n) |>
//            mapFragmentElements(fun(x => x * (beta / alpha)))) |>
//
//          mapFragmentElements(fun(x => x * alpha)) |>
//          fromFragment(n) |> // mTileFrag.nTileFrag.f32
//
...
//  val gemmMultipleFragmentsPerWarp: Expr = {
//    val mTileBlock = 128
//    val nTileBlock = 128
//
//    val mTileFrag = 16
...
//                      aFragTile |> toFragmentA(k)))) |>
//                let(fun(aFrags => // mTileWarp/mTileFrag.WmmaAMatrix
//
//                  //Load tiles of b-matrix into fragments
//                  toPrivate(
//                    abTilesWarp._2 |> transpose |> split(nTileFrag) |>
//                      mapSeqUnroll(fun(bFragTileT =>
//                        bFragTileT |> transpose |> toFragmentB(Col_Major, k)))) |>
//                  let(fun(bFrags => // nTileWarp/nTileFrag.WmmaBMatrix
//
...
//  def blockMMA(loadMatrixFromCIntoFragments: Expr, epilog: Expr): Expr = {
...
//  //Simply use toFragmentAccumulator-primitiv and mapFragmentElemnts to load from global memory into fragments
//  private def loadMatrixFromCIntoFragments(ldmC: Nat): Expr =
...
//            frag |>
//            toFragmentAccumulator(config.nTileBlock + 4) |>
//            mapFragmentElements(fun(x => x * factor)))))) |>
//        transpose)) |>
//
//      join |>
...
//          resultFrag |>
//          mapFragmentElements(fun(x => x * alpha)) |>
//          fromFragment(ldmResult) |>  // mTileFrag.nTileFrag.f32
...
//              resultFrag |>
//              mapFragmentElements(fun(x => x * alpha)) |>
//              fromFragment(config.nTileBlock + 4) |>
//              transpose)) |>
//            join |>
//            split(config.nTileWarp) |>
//            map(fun(x =>
...
//  //b-matrix is transposed
//  private def deviceGEMM(blockGEMM: Expr): Expr =
//    nFun((m, n, k) => fun(
//      f32 ->: f32 ->: (m `.` k `.` f16) ->: (n `.` k `.` f16) ->: (m `.` n `.` f32) ->: (m `.` n `.` f32)
//    )((alpha, beta, a, bT, c) =>
//      zip(
...
//        blockGEMM(n)(
//          alpha, beta,
//          aRowsBlockBColumnBlockCTileBlock._1._1,
//          aRowsBlockBColumnBlockCTileBlock._1._2,
...
//  def gemmSharedMemory(config: mmConfig): Expr = {
//    this.config = config
//    mmTensor.config = config
//    deviceGEMM(nFun(n => blockMMA(loadMatrixFromCIntoFragments(n), epilog(n))))
//  }
//
//  //More optimzed kernel for gemm using shared memory
//  def gemmSharedMemoryV2(config: mmConfig): Expr = {
//    this.config = config
//    mmTensor.config = config
//    deviceGEMM(nFun(n => blockMMA(loadMatrixFromCIntoFragmentsV2, epilogV2)))
//  }
}
