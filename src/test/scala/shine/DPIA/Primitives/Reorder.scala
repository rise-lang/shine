...
class Reorder extends test_util.Tests {
  test(
    "Simple gather example should generate syntactic valid C code with two one loops"
  ) {
    val e = nFun(n =>
  ...
  test(
    "Simple 2D gather example should generate syntactic valid C code with two two loops"
  ) {
    val e = nFun(n =>
      nFun(m =>
  ...
  test(
    "Simple scatter example should generate syntactic valid C code with two one loops"
  ) {
    val e = nFun(n =>
      fun(ArrayType(n, f32))(xs =>
  ...
  test(
    "Simple 2D scatter example should generate syntactic valid C code with two two loops"
  ) {
    val e = nFun(n =>
      nFun(m =>
        fun(ArrayType(n, ArrayType(m, f32)))(xs =>
