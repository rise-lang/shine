...
class algorithmic extends test_util.Tests {
  ...
  test("lift reduce") {
    ...
    val mapReduce = depLambda[NatKind](
      M,
      depLambda[NatKind](
        N,
    ...
    val reduceMap: Rise =
      depLambda[NatKind](
        ...
        depLambda[NatKind](
          N,
          fun(ArrayType(M, ArrayType(N, f32)))(i =>
            reduce(fun((acc, y) => map(addTuple) $ zip(acc, y)))(
              generate(fun(IndexType(M) ->: f32)(_ => l(0.0f)))
            ) $ transpose(i)
          )
        )
      )
    ...
    val goldMKNVersion1 = goldMKN(
      fun((acc, y) => // y :: (f32, N.f32); acc :: N.f32
        mapSeq(fun(t => fst(t) + snd(t))) $
          zip(acc, mapSeq(fun(bs => bs * fst(y))) $ snd(y))
      )
    )
    ...
    val goldMKNAlternative = goldMKN(
      fun((acc, aBN) => { // akB :: (f32, N.f32); acc :: N.f32
        ...
        map(fun(t => fst(t) + (fst(snd(t)) * snd(snd(t))))) $
          /* N.(f32, (f32, f32))*/
          zip(acc, map(fun(bs => pair(bs, as))) $ BN /*:N.f32*/ )
      })
    )

    infer(goldMKNVersion1)
    val typedGold = infer(goldMKNAlternative)
    val loopMKN = (oncetd(liftReduce) `;` LCNF `;` oncetd(removeTransposePair))
      .apply(mm)
      .get
  ...
  ignore("MM-LoopMKN to MM-LoopKMN") {
    ...
    val op = fun((acc, y) => { // akB :: (f32, N.f32); acc :: N.f32
      map(fun(t => fst(t) + (fst(snd(t)) * snd(snd(t))))) $
        /* N.(f32, (f32, f32))*/
        zip(acc, map(fun(bs => pair(bs, fst(y)))) $ snd(y) /*:N.f32*/ )
    })
            ...
            fun(ArrayType(M, ArrayType(K, f32)))(a =>
              fun(ArrayType(K, ArrayType(N, f32)))(b =>
                reduceSeq(
                  ...
                  // generate zeros :: M.N.f32
                  generate(
                    fun(IndexType(M) ->: ArrayType(N, f32))(_ =>
                      generate(fun(IndexType(N) ->: f32)(_ => l(0.0f)))
                    )
                  )
                ) $
  ...
  ignore("mm tile + reorder") {
    ...
    val lower: Strategy[Rise] = LCNF `;` CNF `;` normalize.apply(
      lowering.mapSeq <+ lowering.reduceSeq
    ) `;` BENF
  ...
  test("reduce rows") {
    ...
    val test = nFun(n =>
      nFun(m =>
        fun(ArrayType(n, ArrayType(m, f32)))(i =>
          reduceSeq(
            fun((y, acc) => map(addT) $ zip(y)(acc)),
            generate(fun(IndexType(m) ->: f32)(_ => l(0.0f)))
          ) $ i
        )
      )
  ...
  test("dot and outer product") {
    ...
    val dot = nFun(n =>
      fun(xsT(n))(xs =>
        fun(ysT(n))(ys => (reduce(add)(l(0.0f)) o map(mulT)) $ zip(xs)(ys))
      )
    )

    val outer = nFun(n =>
      fun(xsT(n))(xs =>
        fun(ysT(n))(ys => map(fun(a => map(fun(b => add(a, b))) $ ys)) $ xs)
      )
    )
