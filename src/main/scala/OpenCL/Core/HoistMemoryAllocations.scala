...
import OpenCL.LowLevelCombinators.OpenCLParFor

object HoistMemoryAllocations {

  type AllocationInfo = (OpenCL.AddressSpace, IdentPhrase[VarType])

  def apply(originalP: Phrase[CommandType]): (Phrase[CommandType], List[AllocationInfo]) = {

    var replacedAllocations = List[AllocationInfo]()

    case class Visitor(parForInfo: List[(OpenCL.ParallelismLevel, IdentPhrase[ExpType], Nat)])
          ...
          case New(dt, addressSpace, f) if addressSpace != OpenCL.PrivateMemory =>
            f match {
              case LambdaPhrase(param, body) =>
                replaceNew(addressSpace.asInstanceOf[OpenCL.AddressSpace],
      ...
      private def replaceNew(addressSpace: OpenCL.AddressSpace,
