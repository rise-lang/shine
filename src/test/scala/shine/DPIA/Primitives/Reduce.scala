...
class Reduce extends test_util.TestsWithExecutor {
  ...
  test("Simple example should generate syntactic valid C code with one loop") {
    val e =
      nFun(n => fun(ArrayType(n, f32))(a => a |> reduceSeq(add)(l(0.0f))))
  ...
  test(
    "oclReduceSeq produces correct result over 2D array using private memory"
  ) {
  ...
  test(
    "Record access to specify initial accumulator value of reduceSeq generates syntactically valid C code"
  ) {
    val n = 8
    val initRecordExp =
    ...
    def e(init: Expr) =
      nFun(n => fun(n `.` f32)(arr => arr |> reduceSeq(fun(_ + _))(init)))
  ...
  test(
    "Record access to specify initial accumulator value of oclReduceSeq produces correct result"
  ) {
    import scala.util.Random
    ...
    def runKernel(initWithRecordAccess: Expr) =
      gen
        .OpenCLKernel(e(initWithRecordAccess))
