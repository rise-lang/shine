    ...
    f(p) match {
      ...
      case c: f.Continue =>
        ...
        val res = (p match {
          ...
          case app: ApplyPhrase[_, T] => ApplyPhrase(apply(app.fun, f), apply(app.arg, f))
          case pair: PairPhrase[_, _] => PairPhrase(apply(pair.fst, f), apply(pair.snd, f))
          case p: Proj1Phrase[T, _] => Proj1Phrase(apply(p.pair, f))
          case p: Proj2Phrase[_, T] => Proj2Phrase(apply(p.pair, f))
          ...
          case c: CommandPattern => c.visitAndRebuild(f)
