...
\begin{figure}[t]
  ...
  \begin{minipage}{1.0\linewidth}
      ...
      \end{mathpar}
      \subcaption{Address Spaces}\label{fig:address-space-kinding}
  \end{minipage}
  ...
  \begin{minipage}{1.0\linewidth}
    \begin{mathpar}
      ...
      \inferrule*
      {\Delta \vdash \delta : \datatype}
      {\Delta \vdash \typush[\delta] : \phrasetype}

      \inferrule*
      {\Delta \vdash \delta : \datatype \\\\ \Delta \vdash \alpha : \addrkind}
      {\Delta \vdash \tymem[\delta, \alpha] : \phrasetype}

      \inferrule*
      {\Delta \vdash \delta : \datatype \\\\ \Delta \vdash \alpha : \addrkind}
      {\Delta \vdash \tyacc[\delta, \alpha] : \phrasetype}
...
\begin{figure*}[t]
  ...
  \begin{minipage}{1.0\linewidth}
    \begin{mathpar}
        ...
        %{\Delta \vdash \alpha : \addrkind \\
        % \Delta \vdash \delta : \datatype \\
        % Not needed because the kinding rules imply the kinds of \alpha and \delta
        {\typd{\Pi}{\Gamma}{P}{\tymem[\delta, \alpha]}}
        {\typd{\Pi}{\Gamma}{P}{\tyexp[\delta]}}

        \inferrule*[right=Push Literal]
        {\typd{\Pi}{\Gamma}{P}{\tyexp[\mathsf{num}]}}
        {\typd{\Pi}{\Gamma}{P}{\typush[\mathsf{num}]}}
    \end{mathpar}
  \end{minipage}
...
% TODO: should we move OpenMP primitives elsewhere?
...
\begin{figure}[H]
  ...
  \begin{minipage}{1.0\linewidth} \begin{tabular*}{\linewidth}{>{$}l<{$}@{\hspace{0.4em}}>{$}c<{$}>{$}l<{$}}
      \underline{\ell}&:&\tyexp[\mathsf{num}] \text{ for all }\mathsf{num} \text{ expressions type conversion to } \typush \text{ possible.}\\
        ...
        \prim{idx} &:&(n : \natkind) \to (\dt : \datatype) \to \tyexp[n.\dt] \to \tyexp[\tyidx[n]] \to \tyexp[\dt] \\ % TODO? order differs from implementation, and is inside imperative primitives
    \end{tabular*}
